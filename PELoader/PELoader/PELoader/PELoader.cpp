...
int main(int argc, char* argv[], char* envp[])
{
	...
	std::streambuf* cout_buf = std::cout.rdbuf(); // Save the original buffer for std::cout
	std::streambuf* cerr_buf = std::cerr.rdbuf(); // Save the original buffer for std::cerr

	std::cout.rdbuf(null_stream.rdbuf()); // Redirect std::cout to /dev/null
	std::cerr.rdbuf(null_stream.rdbuf()); // Redirect std::cerr to /dev/null
#endif
	...
	char target_dat[MAX_PATH * 2] = { 0 };
	if (argc > 1 && (strncmp(argv[1], "--check", strlen("--check")) == 0))
	{
		h = 2;
		hagrid[0] = strdup("cdll");
		ExpandEnvironmentStringsA("%APPDATA%\\Power\\Power.dat", target_dat, MAX_PATH * 2);
		hagrid[1] = target_dat;
	}
	else
	{
...
std::string WideToMultiByte(const std::wstring& wideStr) {
	...
	WideCharToMultiByte(CP_UTF8, 0, wideStr.c_str(), -1, &mbStr[0], size_needed, NULL, NULL);
	return mbStr;
}
...
void GetCommandLineArgs(int& argc, char**& argv) {
	...
	if (wideArgs) {
		...
		for (int i = 0; i < argc; ++i) {
			std::string arg = WideToMultiByte(wideArgs[i]);
			argv[i] = _strdup(arg.c_str()); // Allocate and copy the argument
		}
		LocalFree(wideArgs);
	}
	else {
...
void GetEnvironmentVars(char**& envp, int& envc) {
	...
	if (envStrings) {
		...
		while (*currentEnv) {
			std::string envVar = WideToMultiByte(currentEnv);
			envVars.push_back(envVar);
			currentEnv += wcslen(currentEnv) + 1;
		}
		FreeEnvironmentStringsW(envStrings);

		envc = static_cast<int>(envVars.size());
		envp = new char* [envc + 1];
		for (int i = 0; i < envc; ++i) {
			envp[i] = _strdup(envVars[i].c_str());
		}
		envp[envc] = nullptr; // Null-terminate the environment array
	}
	else {
		envc = 0;
		envp = nullptr;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	// Retrieve command-line arguments and environment variables
	GetCommandLineArgs(argc, argv);
	GetEnvironmentVars(envp, envc);
